...
//     const userId = req.user._id
//     console.log('data:::::', razorpay_order_id, razorpay_payment_id, razorpay_signature, checkoutId, deliveryAddress)
...
//     if (checkoutDoc.paymentMethod !== "razorpay") {
...
//         await cartDoc.save();
//       }
//       console.log('id::', cartDoc)
//     }

//     /* ---------------- CLEANUP ---------------- */
//     await checkout.findByIdAndDelete(checkoutId);

//     sendSuccess(res, 'Order placed successfully', {success: true,
//       orderId: order.orderId}, 200)
 ...
 * WHAT CHANGED & WHY
 *
 * Added mapItemsForOrder() — transforms checkout items into the lean order shape.
 *
 * For studio items:
 *   - design: strips down to previewFront/Back only (no canvas data needed post-order)
 *   - supplierPrintInstructions: passed through as-is (already built in normalizeDesign)
 *   - pricingDetails.breakdown: remapped to remove widthCm/heightCm (already in supplierPrintInstructions)
 *     and rename tier → printTier for consistency
 *
 * For shop items: passed through unchanged.
 *
 * Both createOrder and verifyRazorpayOrder now call mapItemsForOrder() before Order.create().
 */

function mapItemsForOrder(checkoutItems) {
  return checkoutItems.map(item => {
    const base = item.toObject ? item.toObject() : { ...item };

    if (base.productType !== "studio") return base;

    return {
      ...base,

      // Customer-facing: previews only
      design: base.design,

      // supplierPrintInstructions already built in normalizeDesign() — pass through
      supplierPrintInstructions: base.supplierPrintInstructions ?? [],

      // Pricing: strip widthCm/heightCm from breakdown (already in supplierPrintInstructions)
      pricingDetails: {
        basePrice:       base.pricingDetails?.basePrice,
        totalPrintPrice: base.pricingDetails?.totalPrintPrice,
        breakdown: (base.pricingDetails?.breakdown ?? []).map(b => ({
          side:      b.side,
          name:      b.name,
          printTier: b.tier ?? b.printTier, // handle both field names during transition
          area:      b.area,
          price:     b.price,
        })),
      },
...
const createOrder = asyncHandler(async (req, res) => {
  try {
    const { checkoutId, deliveryAddress } = req.body;
    const userId = req.user._id;

    /* ── Resolve checkout ── */
    const checkoutDoc = await checkout
      .findById(checkoutId)
      .populate(["items.product", "items.variant", "appliedCoupon"]);
    ...
    if (appliedCoupon?.couponCode) {
      const result = await validateCoupon(appliedCoupon.couponCode, subTotal, items);
      discountTotal = result.discount;
    }
    ...
    /* ── Address ── */
    const orderAddress = {
      fName: deliveryAddress.fName, lName: deliveryAddress.lName,
      phone: deliveryAddress.phone, streetAddress: deliveryAddress.streetAddress,
      city: deliveryAddress.city, state: deliveryAddress.state,
      zipcode: deliveryAddress.zipcode, label: deliveryAddress.label,
    };
    ...
    /* ── Create Order — map items to lean shape ── */
    const order = await Order.create({
      orderId: `ORDER-${idFormat}`,
      user: userId,
      items: mapItemsForOrder(items), // ← mapped here
      totalAmount:   grandTotal,
      subTotal,
      totalDiscount: discountTotal,
      codFee:        checkoutDoc.paymentMethod === "cod" ? config.COD_FEE : 0,
    ...
    if (checkoutDoc.sourceType === "cart") {
      const cartDoc = await Cart.findById(checkoutDoc.sourceId);
      if (cartDoc) { cartDoc.items = []; await cartDoc.save(); }
    }
...
const verifyRazorpayOrder = async (req, res) => {
  try {
    ...
    sendSuccess(res, "Order placed successfully", { success: true, orderId: order.orderId }, 200);
  } catch (error) {
    logger.error("verify razorpay error:", error.message);
    throw error;
  }
...
module.exports = { createOrder, createRazorpayOrder, verifyRazorpayOrder };